...
export const clientPortalController = {
  ...
  async createReview(request: AuthRequest, response: Response) {
    const user = request.user!;
    ...
    const data = createAppointmentReviewSchema.parse(request.body);

    const result = await clientPortalService.createReview({
      userId: user.id,
      appointmentId: request.params.appointmentId,
      rating: data.rating,
